...
(on-chain, off-chain, attestations, DID frameworks)

Jurisdiction or regulatory logic
(SEC rules, EU Prospectus Regulation, etc.)

Custody mechanics
(broker APIs, reconciliation, settlement guarantees)

Rights issues & spin-offs
(planned extensions)

Debt instruments (bonds, notes)
(separate standard required)

Settlement finality guarantees
(chain-dependent)

These are expected to be layered on top of STC-1, not embedded into it.
